...
{ 
  ...
  NRF24L01_CE=0;
	SPI_CS_Disable; 			//使能24L01			//SPI片选取消  		 	 
}
//检测24L01是否存在
//返回值:0，成功;1，失败	
unsigned char NRF24L01_Check(void)
{
	...
	unsigned char i;
	User_SPI_SetSpeed(SPI_BaudRatePrescaler_4); //spi速度为9Mhz（24L01的最大SPI时钟为10Mhz）   	 
...
{
	unsigned char status,_ctr;	       
  	SPI_CS_Enable;           //使能SPI传输
  	status=SPI_ReadWriteByte(reg);//发送寄存器值(位置),并读取状态值   	   
 	for(unsigned char_ctr=0;_ctr<len;_ctr++)
  pBuf[_ctr]=SPI_ReadWriteByte(0XFF);//读出数据
  	SPI_CS_Disable;       //关闭SPI传输
...
{
	unsigned char sta;
 	User_SPI_SetSpeed(SPI_BaudRatePrescaler_8);//spi速度为9Mhz（24L01的最大SPI时钟为10Mhz）   
	NRF24L01_CE=0;
